...
var Particle = /*#__PURE__*/function () {
  ...
  _createClass(Particle, [{
    ...
    value: function draw(ctx) {
      ctx.beginPath();
      ctx.arc(__classPrivateFieldGet(this, _x), __classPrivateFieldGet(this, _y), __classPrivateFieldGet(this, _size), 0, Math.PI * 2, false);
      ctx.shadowColor = __classPrivateFieldGet(this, _color);
      ctx.shadowBlur = 10;
...
_x = new WeakMap(), _y = new WeakMap(), _size = new WeakMap(), _color = new WeakMap();
...
function init(canvas, ctx) {
  var number = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 100;
  var colors = ['#2185C5', '#778beb', '#FFF6E5', '#FF7F66', '#FFC312', '#1289A7', '#f7d794'];
  var item = [];
  var widthWithBoundery = canvas.width + 800;
  var heightWithBoundery = canvas.height + 800;

  for (var i = 0; i < number; i++) {
    var color = colors[Math.floor(Math.random() * colors.length)];
    var x = Math.random() * widthWithBoundery - widthWithBoundery / 2;
    var y = Math.random() * heightWithBoundery - heightWithBoundery / 2;
    var size = Math.random() * 3 + 1;
    item.push(new Particle_1.default(x, y, size, color));
  }
...
function animate() {
  ...
  if (isClick) {
    ...
    if (particles.length !== 1000) {
      particles = [].concat(_toConsumableArray(particles), _toConsumableArray(init(canvas, ctx, 5)));
    }
  }
